...
! Allocating memory
 ...
 !c ely npt-> npt_alloc
 allocate(strk_bud(npt_alloc),dip_bud(npt_alloc),rake_bud(npt_alloc),rd_wl(npt_alloc))
 allocate(syn_up(npt_alloc),syn_h1(npt_alloc),syn_h2(npt_alloc),fsour(npt_alloc),syn_tmp(npt_alloc))
 allocate(gfun(npt_alloc,3),green_ps(npt_alloc,8),rad_h1(npt_alloc,5),rad_h2(npt_alloc,5),rad_vt(npt_alloc,3))
 ! cely
!
 dr=atan(1.0)/45.0
 df=1./(dt*npt)
...
!
 ...
 !ely npt*256
 allocate(csw(2,npt*256/2), trmd(ngr,ngz), green(npt*256,8,ngr,ngz))
 !ely 
!
